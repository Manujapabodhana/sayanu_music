...
import { Mic, MicOff, Video, VideoOff, PhoneOff, Settings, ChevronDown, ChevronUp } from "lucide-react";
...
export const VideoCall = ({ appId, channelName, token, uid }: { appId: string, channelName: string, token: string, uid?: string | number }) => {
    ...
    // Initialize/Update tracks with selected devices
    const { isLoading: isLoadingMic, localMicrophoneTrack } = useLocalMicrophoneTrack(micOn, { microphoneId: selectedMicrophoneId });
    const { isLoading: isLoadingCam, localCameraTrack } = useLocalCameraTrack(cameraOn, { cameraId: selectedCameraId });
    ...
    return (
        <div className="flex flex-col h-full w-full bg-gray-900 rounded-xl overflow-hidden relative">
            ...
            {/* Chat Overlay */}
            <Chat roomId={channelName} senderName={`Student ${uid || ''}`} />
